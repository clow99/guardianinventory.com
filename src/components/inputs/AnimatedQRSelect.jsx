...
import QRScannerModal from "./QRScannerModal"; // See below for this file!

export default function AnimatedQRSelect({
    label,
    value = [],
    onChange,
    id,
    ...
    // QR code handler: find option by value, select if found
    function handleQRScan(scanned) {
        setQrModalOpen(false);
        const matched = options.find((o) => o.value === scanned);
        if (matched) {
            if (!value.includes(matched.value)) {
                onChange && onChange([...value, matched.value]);
            }
            setDropdownOpen(false);
            setIsFocused(false);
        } else {
            alert("No matching option for this QR code.");
        }
    }
